...
import os
import sys
import requests  # ✅ First: Import requests library
...
if os.path.exists(nltk_data_path):
    ...
    st.error(f"NLTK data not found at {nltk_data_path}. Please ensure the nltk_data folder is included in your deployment.")

# --- Preprocessing Function ---
def preprocess_text(text):
    text = text.lower()
    text = ''.join([char for char in text if char.isalnum() or char.isspace()])
    tokens = nltk.wordpunct_tokenize(text)  # ✅ Second: Switched to wordpunct_tokenize
